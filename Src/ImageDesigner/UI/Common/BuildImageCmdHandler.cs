...
internal class BuildImageCmdHandler : CmdHandler
{
  ...
  {
    this._cmdLineParser.SetRequiredParameterString("OutputFile", 
        "Path to the OEM Input XML file");

    this._cmdLineParser.SetRequiredParameterString("OemInputXml", 
        "Path to the OEM Input XML file");

    this._cmdLineParser.SetOptionalSwitchString("CustomizationFile", 
        "Path to the customization file");

    this._cmdLineParser.SetOptionalSwitchString("MSPackageRoot",
        "Path to the folder containing Microsoft base packages");

    this._cmdLineParser.SetOptionalSwitchString("BSPConfigFile", 
        "Path to the BSPConfig.xml file under BSP root");
  }
  ...
  protected override int DoExecution()
  {
    ...
    if (this._cmdLineParser.IsAssignedSwitch("BSPConfigFile"))
    {
      ...
      this._imageBuilder.BSPRoot = BSPConfig.LoadFromXml(switchAsString, 
          this._imageBuilder.MSPackageRoot).BSPRoot;
    }
