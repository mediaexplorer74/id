...
internal class FlashImageCmdHandler : CmdHandler
{
  ...
  public FlashImageCmdHandler() => this._cmdLineParser.SetRequiredParameterString("ImageFile", 
      "Path to the image file (.FFU)");
  ...
  protected override int DoExecution()
  {
    ...
    DeviceFlasher deviceFlasher = 
            Microsoft.WindowsPhone.ImageUpdate.Tools.Common.LongPathFile.Exists(parameterAsString)
            ? new DeviceFlasher(parameterAsString)
            : throw new WPIDException((Exception) new FileNotFoundException(
                string.Format("FFUImage {0} not found", (object) parameterAsString), 
                parameterAsString), "Specified FFU file does not exist");
