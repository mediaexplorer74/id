  ...
  internal class ImageDesignerMain
  {
    ...
    public static void Main(string[] args)
    {
      if (args.Length == 0 || args[0].Equals("-Environment", StringComparison.OrdinalIgnoreCase))
      {
        ...
        if (args.Length == 1)
        {
          CustomMessageBox.ShowMessage(Microsoft.WindowsPhone.ImageDesigner.Core.Tools.GetString("txtBadArgumentTitle"), string.Format((IFormatProvider) CultureInfo.CurrentCulture, Microsoft.WindowsPhone.ImageDesigner.Core.Tools.GetString("txtInvalidArgument")));
        ...
        else
        {
          ...
          {
            CustomMessageBox.ShowMessage(Microsoft.WindowsPhone.ImageDesigner.Core.Tools.GetString("txtMissingEnvironmentVariableTitle"), string.Format((IFormatProvider) CultureInfo.CurrentCulture, Microsoft.WindowsPhone.ImageDesigner.Core.Tools.GetString("txtWpdkContentRootNotSet")));
            Application.Current.Shutdown(-1);
          }
          else
          {
