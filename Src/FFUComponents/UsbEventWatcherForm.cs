...
namespace FFUComponents
{
  internal class UsbEventWatcherForm : Form
  {
    ...
    private unsafe string GetUsbDevicePath(string pnpId)
    {
      ...
      DeviceInterfaceDetailData* interfaceDetailDataPtr = (DeviceInterfaceDetailData*) (void*) Marshal.AllocHGlobal(requiredSize);
      try
      {
        interfaceDetailDataPtr->Size = IntPtr.Size != 4 ? 8 : 6;
        DeviceInformationData deviceInfoData = new DeviceInformationData()
        {
          Size = Marshal.SizeOf(typeof (DeviceInformationData))
        };
        if (!NativeMethods.SetupDiGetDeviceInterfaceDetail(classDevs, ref deviceInterfaceData, interfaceDetailDataPtr, requiredSize, ref requiredSize, ref deviceInfoData))
          throw new Win32Exception(Marshal.GetLastWin32Error());
        return Marshal.PtrToStringAuto(new IntPtr((void*) &interfaceDetailDataPtr->DevicePath));
      }
      finally
      {
        Marshal.FreeHGlobal((IntPtr) (void*) interfaceDetailDataPtr);
      }
    }
    ...
    private IntPtr RegisterDeviceNotification(Guid ifGuid)
    {
      ...
      IntPtr num = Marshal.AllocHGlobal(structure.Size);
      try
      {
