	...
	public class CabApiWrapper
	{
		...
		public static string[] GetFileList(string filename, out ulong[] fileSizes)
		{
			...
			try
			{
				...
				long[] array1 = new long[num];
				Marshal.Copy(zero2, array1, 0, (int)num);

				//Example of long[]-> ulong[]
                //long[] result = new long[16];
                //ulong[] state = Unsafe.As<long[], ulong[]>(ref result);
                fileSizes = Unsafe.As<long[], ulong[]>(ref array1);  // (ulong[])array1
